...
@testset "PyMRIOParser Parser Rules" begin
    ...
    @testset "__construct_IO" begin
        ...
        res = P.__construct_IO(data_sut; construct="B")
        ...
        res_empty = P.__construct_IO(data_empty; construct="B")
        @test res_empty isa Dict{String, SparseMatrixCSC{Float64, Int}}
        @test size(res_empty["A"]) == (0, 0)
        
        # 3. Invalid/malformed inputs
        data_missing = Dict("VA" => V)
        @test_throws KeyError P.__construct_IO(data_missing; construct="B")
