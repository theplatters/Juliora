...
function parse_gloria(path::String, year::Int; version=60, price=BasePrice(), country_names="gloria")
  ...
  qy_suffix = gloria_satellite_suffix("Y", year, version)

  Q_SUT, QY_SUT = read_satellites(is_unzipped, sat_path, q_suffix, qy_suffix; n_regions=n_regions, n_sectors=n_sectors)

  @info "Constructing MRIO"
  return _construct_IO(S, U, Y, VA, regions, sectors, va_cats, fd_cats, Q_SUT, QY_SUT, sat_df)
end

function read_satellites(::Type{Zipped}, sat_path, q_suffix, qy_suffix; n_regions, n_sectors)
  return open(sat_path, "r") do io
    mmap_data = Mmap.mmap(io)
    sat_zip = za.ZipReader(mmap_data)

    q_bytes = read_entry_in_zip(sat_zip, q_suffix)
    qy_bytes = read_entry_in_zip(sat_zip, qy_suffix)

    return (parse(QFile(), q_bytes; n_regions=n_regions, n_sectors=n_sectors), parse(QYFile(), qy_bytes; n_regions=n_regions, n_sectors=n_sectors))
  end
end

function read_satellites(::Type{Unzipped}, sat_path, q_suffix, qy_suffix; n_regions, n_sectors)
  q_file = find_file_in_dir(sat_path, q_suffix)
  qy_file = find_file_in_dir(sat_path, qy_suffix)

  q_bytes = mmap_file(q_file)
  qy_bytes = mmap_file(qy_file)
