...
function from_long_dataframe(
    ...
    for row in eachrow(df)
        ...
        row_vals = NamedTuple{Tuple(clean_row_cols)}(Tuple(row[col] for col in row_cols))
        col_vals = NamedTuple{Tuple(clean_col_cols)}(Tuple(row[col] for col in col_cols))
...
function environmental_impact(mrio::MRIO, production::AbstractVector{<:Real})
    return environmental_impact(mrio.env, production)
end
